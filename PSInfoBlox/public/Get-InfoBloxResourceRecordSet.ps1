Function Get-InfoBloxResourceRecordSet {
    ...
    param(
        ...
        [ValidateSet("A","AAAA","CName","Host","MX","NAPTR","PTR","SRV","TXT","All")]
        [string[]]
        $RecordType = "A",
    ...
    BEGIN {
		...
		# The following list seems to send errors, using only list that doesn't throw errors.
		# $RecordTypes = @("A","AAAA","CName","DName","DNSKEY","DS","Host","LBDN","MX","NAPTR","NS","NSEC","NSEC3","NSEC3PARAM","PTR","RRSIG","SRV","TXT")
		$RecordTypes = @("A","AAAA","CName","Host","MX","NAPTR","PTR","SRV","TXT")
        ...
        if ( $PSCmdlet.ParameterSetName -eq "Credential" ) {
			...
			elseif ( -not([string]::IsNullOrEmpty($Script:IBConfig.IBVersion)) ){
				$Params.Add('IBVersion',$Script:IBConfig.IBVersion)
			}
            
            if ( $PSBoundParameters.ContainsKey("IBServer")) {
                 $Params.Add('IBServer',$IBServer) 
            }
			elseif ( -not([string]::IsNullOrEmpty($Script:IBConfig.IBServer)) ){
				$Params.Add('IBServer',$Script:IBConfig.IBServer)
			}
    ...
    PROCESS {
		...
		ForEach ( $SingleRecordType in $RecordType ) {
			$BaseUri = "{0}/record:{1}" -f $Uri, $SingleRecordType.ToLower()
			...
			if ( $PSBoundParameters.ContainsKey("SearchValue") ) {
				...
				if ( $SearchValueAliasUsed ) {
					...
					$ReqUri = "{0}&{1}{2}{3}" -f $ReqUri, $SearchField.ToLower(), $EqualityOperator, $SearchValue
				}
				else {
					Write-Verbose "Not using specific equality operator"
					switch ( $SearchField ) {
						{$exactQualityArr -contains $_} {$ReqUri = "{0}&{1}={2}" -f $ReqUri, $SearchField.ToLower(), $SearchValue}
						{$regexArr -contains $_} {$ReqUri = "{0}&{1}~={2}" -f $ReqUri, $SearchField.ToLower(), $SearchValue}
						{$caseInsensitiveArr -contains $_} {$ReqUri = "{0}&{1}:={2}" -f $ReqUri, $SearchField.ToLower(), $SearchValue}
					}
				}
			}
